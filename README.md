...
```js
{
  ...
  gradeTerm: "4-1",
  category: "整数のたし算",       // "整数のたし算" / "整数のひき算" / "整数のかけ算" / "1けたでわるわり算"
...
- カウントダウン画面（3・2・1・START!）。数字の下に「式をつくって　＝ボタンで答えよう！」
  という遊び方のヒント文を常に表示する（運用開始後に追加。`runCountdown()`は文章題バトル・
  トレーニング・総復習の3モードで共有しているため、モードを問わず同じ文言を表示する）
...
- 敵を倒したときの点滅して消える演出
- スコア・ランク計算、ノーミス判定（+表示）
- **特別ランク「MAX」**（文章題バトル専用。運用開始後に追加）: 全問、タイムボーナスが上限
  （`TIME_BONUS_MAX`＝20。1段階問題なら4秒以内に正解、2段階問題は7秒以内（4秒×1.75倍の
  係数）、3段階問題は10秒以内（4秒×2.5倍の係数）に正解、が条件）で正解できた場合だけの、
  理論値スコアの証。1問でも不正解・時間切れ・上記の時間を超えて正解した問題があると、
  その時点で対象外になり、以後そのゲームでは復活しない（`gameState.isMaxBonusRun`、
  `js/game.js`）。条件を満たしたまま最終問題に正解した瞬間、バトル中のランクカード
  （スコア表示の右）が通常のランク文字（例:「SS」「S+」）ではなく「MAX」に切り替わり、
  枠が虹色グラデーションで光る演出（`.rank-max`、`css/style.css`）になる。結果画面の
  「今回のランク」にも同じ「MAX」表示・虹色演出を反映する。スコア計算式・ランクの閾値
  （`calculateRank()`）・ハイスコア判定・エネミー図鑑の解放条件など、既存のスコア関連仕様は
  一切変更していない（「MAX」はあくまで表示だけの特別な上書きで、内部的には通常どおりの
  ランク文字を保持し続ける）
...
- **整数のみの2段階（2つの式で解く）文章題**（`questionType: "multiStep"`）
  ...
  - 途中式正解時はタイマー（解答時間ゲージ）を全回復させず、一時停止→再開する。ゲージの回復量は
    「途中式正解」の演出が表示されたタイミングで即座に反映され（演出が終わって次の式へ進むときに
    まとめて回復するのではない）、演出の間はその回復済みの値のままタイマーが止まっており、
    演出が終わると回復後の値からカウントダウンを再開する（運用開始後に変更。以前は演出終了時に
    回復していた）
...
- **トレーニングモード（第6段階、新規）**
  ...
  - **「同じ問題をもう一度」ボタン**（運用開始後に追加）: 正解した直後、通常の「タップして次へ」
    に加えて表示される、トレーニング専用の2つ目のボタン（`.training-only`。バトル・総復習の
    正解演出には一切登場せず、CSSの`display:none`で完全に非表示・操作不能になる）。押すと、
    その正解によって加算された完了数・1回で正解数・問題履歴を取り消したうえで、同じ問題を
    答える前の状態（2段階問題なら、1つ目の式を答える前の状態。`js/multi-step-engine.js`の
    `initializeMultiStepQuestion()`を再度呼んで中間結果・候補ルート・選択肢カードをリセット
...
- **小学6年生・3学期（第12段階、正式モード、`gradeTerm: "6-3"`）**
  ...
  - 縮尺・地図上の長さ（縮尺と実際の長さから、地図上の長さ（cm）を求める2段階問題。
    「実際の長さをcmへ単位変換してから縮尺の分母で割る」ルートに加えて、`g6t3_scale_map_002`
    （公園の案内図）・`g6t3_scale_map_003`（学校の敷地の案内図）だけ「先に縮尺の分母で
    割ってから単位変換する」ルート（例: 55÷500=0.11(m)→0.11×100=11(cm)）にも対応
    （運用開始後に追加）。2つの正解ルートを登録すると、最初の式の選択肢カードにも両方の
    ルートに必要な数値（実際の長さ・単位変換の倍率・縮尺の分母）が自動的にすべて含まれる
    （`js/multi-step-engine.js`のbuildChoicesForActiveStep()が、候補として残っている
    すべての正解ルートの現在の式を見て選択肢を組み立てる汎用設計のため、テンプレート側で
    個別に選択肢を調整する必要が無い）。
    `js/question-validator.js`のvalidateScaleLength()も、この2つの生成タイプ
    （findActualLengthFromScale・findMapLengthFromScale）で正解ルートを1〜2個まで
    許容するよう更新した（ルートidは生成タイプごとに異なるため、特定のidを必須とはせず、
    id重複が無いことだけを検証する）
  - 比例定数・反比例の一定の積は、独立したカテゴリとしては追加せず、式1で児童が求める
    中間結果として扱う（問題文には一切表示しない。`hiddenIntermediateKeys` で検証する）
  - 縮尺の分母が単位変換の倍率（`actualLengthUnit`が"m"なら100、"km"なら100000）と
    ちょうど一致する値は、縮尺の分母選択の候補から除外する（運用開始後に追加）。一致すると
    計算上その倍率が打ち消し合い、地図上の長さと実際の長さの「数字」だけが単位を無視すると
    偶然一致してしまうため（例: 縮尺1：100,000で実際の長さ6kmなら地図上の長さも6cmになる。
    `mapLengthValue`の値によらず常に起こる）。`js/question-generator.js`の
    `generateScaleLengthValues()`が、12種類のテンプレートすべてで共有するこの生成ルールの中で
    対応しており、個々のテンプレートの縮尺の分母の一覧（`variables.scaleDenominator.values`）
    自体は変更していない
  ...
  - 比例・反比例の一部のテンプレートは、対応関係を表す**関係表**（HTMLの`table`要素、見出しに`th`。
    `relationTable`。児童が求める値のセルは「？」として表示し、答えを漏らさない設計）を表示できる
    仕組みを持つが、**現在はどのテンプレートにも`relationTable`を設定しておらず、問題文のみを
    表示する**（運用開始後に変更。以前は`g6t3_proportion_001`・`g6t3_proportion_004`・
    `g6t3_inverse_001`・`g6t3_inverse_002`の4テンプレートに関係表があったが、問題文だけの表示に
    統一するため削除した。`js/question-generator.js`・`js/ui.js`・`js/value-renderer.js`・
    `js/question-validator.js`側の関係表を描画・検証する仕組み自体は、将来また使う可能性を
    考慮してそのまま残してある）
...
- **第3のモード「総復習」**（運用開始後に追加。「文章題バトル」「トレーニング」に続く3つ目の
  ...
  - 総復習を選ぶと、サブメニューとして「4年のまとめ」「5年のまとめ」「6年のまとめ」
    ...
    「ハートは○個で ○問正解したらクリア！」（2行目、ハート数・必要正解数はスコープに応じて
    動的に変わる）の2行のテキストと「スタート」「もどる」ボタンが表示される
    （運用開始後に変更。以前は2行目が「全部で○問です。」だった）
...
- **ヘルプ機能・エネミー図鑑**（運用開始後に追加）
  ...
  - ヘルプは、問題・スコア・ハート等のゲーム進行を一切持たないタイトル画面の補助画面のため、
    ...
    効果音設定は一切リセットされない
  - **「⚠記録を消す」（運用開始後に追加）**: ヘルプメニューの「エネミー図鑑」の下、「もどる」の
    上に配置。押すと確認ダイアログが開き、2回連続で「⚠消去する」を押した場合だけ実際に消去する
    2段階確認になっている（誤操作防止）。1回目は「すべてのハイスコアとエネミー図鑑の情報を
    消去します。よろしいですか？」という通常表示、1回目の「⚠消去する」を押すと同じダイアログの
    文言が「本当によろしいですか？後悔しませんね？」という赤文字（`.dialog-warning-text`）に
    切り替わり、ここで「もどる」を押せば何も消さずに段階1へ戻る。2回目の「⚠消去する」を押した
    ときだけ `js/storage.js` の `resetHighScoresAndEnemyDex()` を実行し、`highScore_` で始まる
    キー（すべての出題範囲・レベルの組み合わせのハイスコア）と `mathWordBattle_defeatedEnemyIds`
    （エネミー図鑑の解放状態）を削除する。効果音設定・前回選んだ出題範囲/モード・トレーニングの
    選択学期・6年3学期の出題グループローテーション位置など、それ以外の保存データには一切触れない。
...
- 4つ以上の式が必要な問題、括弧を使う式
- 関係表（`relationTable`）の表示（運用開始後に削除。以前は比例・反比例の一部のテンプレートに
  持たせていたが、問題文だけの表示に統一したため、現在はどのテンプレートにも設定していません。
  詳しくは[「比例・反比例の関係表（relationTable）」の節](#比例反比例の関係表relationtable)を参照）
...
- [ ] カウントダウン画面で、数字（3・2・1・START!）の下に「式をつくって　＝ボタンで
      答えよう！」というヒント文が表示される（トレーニング・総復習のカウントダウンでも同様）
...
- [ ] 文章題バトルで、すべての問題を4秒以内（多段階問題は段階数に応じた時間、2段階なら7秒、
      3段階なら10秒）に正解すると、最終問題に正解した瞬間、バトル中のランクカードが
      通常のランク文字ではなく「MAX」に切り替わり、虹色グラデーションで光る
- [ ] 1問でも不正解・時間切れ・上記の時間を超えて正解した問題があると、全問正解してクリアしても
      「MAX」にはならず、通常どおりのランク文字（例:「SS」「S+」）が表示される
- [ ] 「MAX」でクリアしたときも、結果画面の「今回のランク」に同じ「MAX」表示・虹色演出が反映される
- [ ] 「MAX」表示中も、スコア・ハイスコア判定・エネミー図鑑の解放は通常どおり正しく動作する
      （回帰確認）
- [ ] トレーニング・総復習モードには「MAX」ランクが存在しない（元々スコア・ランクの概念がない）
...
- [ ] 途中式正解時、解答時間ゲージは「途中式正解」の演出が表示されたその瞬間に回復済みの値へ
      切り替わる（演出が消えるタイミングまで回復が遅れない）
- [ ] 「途中式正解」の演出が表示されている間（約0.9秒）、ゲージの値はそのまま変化せず、
      演出が終わったあとに、その回復済みの値からタイマーのカウントダウンが再開する
...
- [ ] 比例・対応する量／反比例・対応する量のどの問題でも、関係表（`table`要素）が表示されず、
      問題文だけが表示される（運用開始後に変更。以前は一部の問題で関係表も表示していた）
- [ ] 縮尺・実際の長さの答えが正しい（例: 縮尺1：25,000、地図上4cm→実際の長さ`4×25000=100000cm=1km`）
- [ ] 縮尺・地図上の長さの答えが正しい（例: 縮尺1：50,000、実際の長さ2km→`2km=200000cm`、
      `200000÷50000=4cm`）
- [ ] `g6t3_scale_map_003`（学校の敷地の案内図）で、「実際の長さ×100÷縮尺の分母」だけでなく
      「実際の長さ÷縮尺の分母×100」の順序で式を組み立てても正解と判定される
      （例: 縮尺1：500、実際の長さ55m→`55÷500=0.11(m)`→`0.11×100=11(cm)`）
- [ ] `g6t3_scale_map_002`（公園の案内図）でも同様に、どちらの順序でも正解と判定される
      （例: 縮尺1：800、実際の長さ48m→`48÷800=0.06(m)`→`0.06×100=6(cm)`）
- [ ] `g6t3_scale_map_002`で縮尺1：800・実際の長さ48mの問題が出た場合、1つ目の式の選択肢
      カード（数値4枚）に「48」「100」「800」がすべて含まれている（どちらの解き方でも
      式を組み立てられる）
- [ ] 縮尺・実際の長さ／縮尺・地図上の長さ（12種類すべて）で、縮尺の分母が単位変換の倍率
      （mの問題なら100、kmの問題なら100000）そのものになることが無い（`?debug=true`の
      デバッグパネルの`元の数値データ`で`scaleDenominator`を確認できる）
- [ ] 縮尺・実際の長さ／縮尺・地図上の長さ（12種類すべて）で、地図上の長さと実際の長さの
      数字が単位を無視すると偶然一致する、ということが起きない
      （例: 縮尺1：100,000で実際の長さ6kmのとき、地図上の長さが6cmになるような問題が出題されない）
...
- [ ] ページ全体が横スクロールしない。スマートフォン縦画面でも縮尺の表示が崩れない
...
- [ ] いずれかのボタンをタップすると、確認ダイアログが開き「○年のまとめを はじめますか？」
      （1行目）「ハートは○個で ○問正解したらクリア！」（2行目）が改行して表示される
      （4年＝ハート3個・13問、5年＝ハート3個・17問、6年＝ハート3個・20問、
      小学校＝ハート1個・50問）
...
- [ ] ヘルプメニューに「エネミー図鑑」の下・「もどる」の上の位置で「⚠記録を消す」ボタンが表示され、
      赤い枠線で他のボタンと視覚的に区別できる
- [ ] 「⚠記録を消す」を押すと「すべてのハイスコアとエネミー図鑑の情報を消去します。
      よろしいですか？」という確認ダイアログが、赤文字にならずに表示される
- [ ] 1回目の「⚠消去する」を押すと、まだ何も消去されず、ダイアログの文言だけが「本当に
      よろしいですか？後悔しませんね？」という赤文字（`.dialog-warning-text`）に切り替わる
- [ ] 2段階目（赤文字）の表示中に「もどる」を押すと、何も消去されずにダイアログが閉じ、
      次に「⚠記録を消す」を押したときは1段階目の通常表示から始まる（段階がリセットされている）
- [ ] 2段階目（赤文字）の表示中に「⚠消去する」をもう一度押すと、ハイスコア・エネミー図鑑の
      解放状態がすべて即座に消去される
- [ ] 消去した直後は「⚠消去する」「もどる」ボタンが隠れ、「すべての記録を消去しました。」
      という完了メッセージだけが表示される
- [ ] 完了メッセージが表示されてから2.5秒経つと、自動的にダイアログが閉じてヘルプメニュー画面に
      戻り、「⚠記録を消す」ボタンへフォーカスが戻る
- [ ] 完了メッセージの表示中に、ボタンが隠れているため誤って再度消去を実行できない
- [ ] 消去後にもう一度「⚠記録を消す」を押すと、2段階確認の1段階目（通常表示）から始まる
      （内部の確認段階が正しくリセットされている）
- [ ] 消去後、エネミー図鑑を開くと「発見したエネミー　0／26」に戻り、すべてのカードが
      未解放（❓／？？？）表示になる
- [ ] 消去後、タイトル画面のハイスコア表示もリセットされている
- [ ] 消去を実行しても、効果音設定・前回選んだ出題範囲/モード・トレーニングの選択学期・
      6年3学期の出題グループローテーション位置など、それ以外の保存データは変化しない
- [ ] 消去後にページを再読み込みしても、消去された状態（ハイスコア0・図鑑0／26）のまま維持される
...
- [ ] 関係表（`relationTable`）を追加する場合、`rowHeaders`（行見出しの配列）と `columns`
      ...
      `{type:"unknown"}`）の形式になっている（運用開始後に変更。以前は比例・反比例テンプレート
      全体のうち少なくとも3分の1程度に関係表を持たせる方針だったが、問題文だけの表示に統一する
      ため全テンプレートから削除した。現時点では`relationTable`を持つテンプレートは無い）
...
に統一し、トレーニングでは「2つの式の文章題」という1つのカテゴリとしてまとめて出題します。
...
- [ ] 正解すると「タップして次へ」に加えて「同じ問題をもう一度」ボタンが表示される
      （トレーニングモードのみ。文章題バトル・総復習の正解演出には表示されない）
- [ ] 「同じ問題をもう一度」を押すと、同じ問題文・同じ問題番号のまま、解答欄・選択肢カードが
      未回答の状態に戻り、もう一度最初から解ける
- [ ] 2段階問題で最終（2つ目）の式に正解したあと「同じ問題をもう一度」を押すと、2つ目の式では
      なく「式を2つ答えよう！」（1つ目の式）から再び答えることになる
- [ ] 「同じ問題をもう一度」を押しても、完了数・1回で正解した数・問題履歴が加算されたままには
      ならない（`?debug=true`のデバッグパネルの`trainingCompletedQuestions`で確認できる）
- [ ] 「同じ問題をもう一度」を何度でも繰り返し使え、そのたびに完了数が加算されたままにならない
...
**大カテゴリ（学年・学期）チェックボックス（運用開始後に追加）**: 各グループの見出し
（`.custom-training-group-heading`）自体にもチェックボックスを持たせ、配下の個別カテゴリと同じ
`.custom-training-group`（`gradeTerm` ごとのコンテナ）内に入れ子で配置しています。
- 大カテゴリのチェックボックスをONにすると、配下のカテゴリがすべてONになる
- 大カテゴリのチェックボックスをOFFにすると、配下のカテゴリがすべてOFFになる
- 配下のカテゴリを1つでもOFFにすると、大カテゴリのチェックボックスも自動でOFFになる
- 逆に、配下のカテゴリを1つずつ手動でONにしていき、すべてONになると、大カテゴリのチェックボックス
  も自動でONになる

（`js/ui.js` の `handleCustomTrainingGroupCheckboxChange()` / `handleCustomTrainingCategoryCheckboxChange()`）。
...
問題画面のヘッダー（カテゴリ名表示）は、カスタムトレーニングでは「カスタムトレーニング」という
総称だけを表示し、今の問題自身のカテゴリ名は表示しません（運用開始後に修正。以前は問題ごとの
カテゴリ表示名を出していましたが、複数カテゴリから出題されるカスタムトレーニングにおいて、
答える前に「今の問題が何のカテゴリか」を明かすことが実質的なヒントになってしまうという指摘を
受けて変更しました）。結果画面・デバッグパネル用の `trainingState.categoryLabel`（「カスタム
トレーニング（8カテゴリ）」のようなカテゴリ数つきのサマリー文言）とは別に、ヘッダー表示専用に
`updateTrainingHeaderDisplay()`（`js/training-mode.js`）がカテゴリ数を含まない文言を渡します。
なお、通常トレーニングのヘッダーには、従来どおり選択したカテゴリの表示名がそのまま表示されます。
...
- 画面下部のボタンは、選択中カテゴリ数に応じて「すべてON」／「すべてOFF」に文言・挙動が
  切り替わります（運用開始後に変更。以前は常に全解除だけを行う「設定リセット」ボタンでした）。
  選択中が0件のときは「すべてON」（押すと全カテゴリの画面上のチェック・メモリ上の選択・
  保存データをすべてONにする）、1件以上あるときは「すべてOFF」（押すとすべて空にする。
  `clearCustomTrainingCategoryIds()`）になります。
...
- [ ] 学年・学期の見出し自体にもチェックボックスが表示される
- [ ] 見出しのチェックボックスをONにすると、配下のカテゴリがすべてONになる
- [ ] 見出しのチェックボックスがONの状態で、配下のカテゴリを1つでもOFFにすると、見出しの
      チェックボックスも自動でOFFになる
- [ ] 見出しのチェックボックスがONの状態で、見出し自体のチェックを外すと、配下のカテゴリが
      すべてOFFになる
- [ ] 配下のカテゴリを1つずつ手動でONにしていき、すべてONになると、見出しのチェックボックスも
      自動でONになる
- [ ] カテゴリ一覧部分だけが縦スクロールでき、見出し（タイトル・もどる）とフッター
      （すべてON/OFFボタン・スタート）は常に見える
- [ ] 選択カテゴリ数（「選択中：○カテゴリ」）が、チェックを変更するたびに即座に更新される
- [ ] 初期状態は0カテゴリで、この状態ではスタートボタンが無効になっている
- [ ] 1カテゴリ以上選ぶとスタートボタンが有効になる
- [ ] 選択中0カテゴリのとき、画面下部のボタンが「すべてON」と表示され、押すと全カテゴリの
      チェックがONになり、選択中：（全カテゴリ数）カテゴリになる
- [ ] 選択中1カテゴリ以上のとき、画面下部のボタンが「すべてOFF」と表示され、押すとすべての
      カテゴリのチェックが外れ、選択中：0カテゴリに戻り、保存データも空になる
      （問題数スライダーは変更されない）
...
- [ ] 問題画面のヘッダーには「カスタムトレーニング」という総称だけが表示され、今の問題自身の
      カテゴリ名は表示されない（ヒントにならないよう、運用開始後に変更）
...
| ランク係数 | `現在のスコア÷レベル÷(レベル×60+1640)` | `js/score.js` の `calculateRank()`。以前の開発段階で800→1000→1600（×レベルで割る方式）へ、さらに`レベル÷(レベル×60+1640)`で割る方式へ変更済み |
...
```js
{ id: "unlike-fraction-addition", label: "分数のたし算", gradeTerm: "5-2", gradeLabel: "小学5年生・2学期", enabledInTraining: true, order: 18 },
{ id: "unlike-fraction-subtraction", label: "分数のひき算", gradeTerm: "5-2", gradeLabel: "小学5年生・2学期", enabledInTraining: true, order: 19 },
...
| ランク係数 | `現在のスコア÷レベル÷(レベル×60+1640)`（MAXは`÷12000`。以前の開発段階では`÷(1600×レベル)`だったが、その後変更済み） |
...
2種類ずつ（全体の約1/3）に関係表を持たせていました。

**運用開始後に変更**: 問題文だけの表示に統一するため、`g6t3_proportion_001`・
`g6t3_proportion_004`・`g6t3_inverse_001`・`g6t3_inverse_002`の`relationTable`フィールドを
すべて削除しました。`resolveRelationTable()`は`template.relationTable`が無ければ何もせず
`problem.relationTable`は`null`になり、`renderRelationTable()`（`js/ui.js`）はその場合
`#relation-table-container`を空にして`aria-hidden="true"`にするだけなので、テンプレート側の
フィールドを消すだけで安全に「表を表示しない」状態にできました（`js/question-generator.js`・
`js/ui.js`・`js/value-renderer.js`・`js/question-validator.js`側の関係表を描画・検証する仕組み
自体は削除していません）。

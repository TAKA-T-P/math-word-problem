...
縮尺・実際の長さ、縮尺・地図上の長さ、縮尺を求める）を通常バトルの正式モードとして追加**しました。
...
タイマー・ハート・敵HP・スコアの無い「トレーニングモード」では、51種類のカテゴリ（単元）から1つを選び、
...
│  ├─ enemy-list.js              … 出現するエネミー24種類の絵文字・名前・出現レベル・クリア/ゲームオーバー時のせりふ・キャラ紹介文の単一の情報源。game.js（抽選）・ui.js（タイトル画面のプレビュー・結果画面のエネミーコメント）から参照する
...
│  └─ storage.js                 … ハイスコア・効果音設定・選択中の出題範囲の保存/読み込み・6年3学期の出題グループローテーション位置（第12段階で追加）
├─ data/
│  ├─ index.js                  … 問題データ読み込みの一元窓口。ゲーム本体はここ経由でのみ取得する
│  ├─ category-registry.js      … トレーニングで選べる51カテゴリの単一の情報源（id/表示名/学期/表示可否/表示順。第12段階で5カテゴリ追加）
...
│  ├─ grade6-term3.js           … 小学6年生・3学期の問題テンプレート（30種類、比例・対応する量/反比例・対応する量/縮尺・実際の長さ/縮尺・地図上の長さ/縮尺を求める。第12段階で新規）
│  └─ multi-step-integer.js     … 整数のみの2段階問題テンプレート（12種類。開発版モード「4-multi-step」、4-2モードの「2段階文章題」カテゴリ、4-3/5-1/5-2/5-3/6-1/6-2モードの復習内容から共有）
└─ tools/
...
- `resultType`（省略可、第9段階で追加）: ステップの計算結果を別の型に変換したい場合に指定します。
  ...
  - `"scaleDenominator"`（第12段階で追加）: 数値÷数値の結果（縮尺の分母にあたる整数）を、
    縮尺の値オブジェクト（例: `{type:"scale", numerator:1, denominator:25000}`）に変換します
    （`createScale()`）。「縮尺を求める」（実際の長さ÷地図上の長さ＝縮尺の分母）のように、
    計算自体は普通の整数の割り算だが**表示だけ縮尺（1：n）にしたい**場合に使います。
    `"percent"` と同じ考え方の変換ですが、変換先が百分率ではなく縮尺という点だけが異なります。
...
- 結果画面での**エネミーコメント表示**（運用開始後に追加。結果タイトルのすぐ下に、その回のバトルに
  ...
  用意しており、単一の情報源（`js/enemy-list.js`）はキャラ紹介文も保持しているが、紹介文は
  今回のバージョンでは画面に表示せず、将来の「エネミー図鑑」機能のためのデータとして温存している）
...
- **小学6年生・2学期（第11段階、正式モード、`gradeTerm: "6-2"`）**
  ...
  - 1〜3学期＋5年1〜3学期＋6年1学期の復習内容と6年2学期の新出内容を同数（新内容:復習=1:1）で出題
  - 新内容側は8カテゴリに均等に近い頻度で配分（ラウンドロビン、8カテゴリ以下なので出題数の範囲内では重複しない）
  - 復習内容は4-1/4-2/4-3/4-multi-step/5-1/5-2/5-3/6-1から偏りなく選ばれる
  - 同じ問題文・同じ式が1回のゲーム内で繰り返し出題されないようにする重複回避
...
- **小学6年生・3学期（第12段階、正式モード、`gradeTerm: "6-3"`）**
  ...
  - 縮尺を求める（地図上の長さと実際の長さから、縮尺（`1：n`）そのものを求める2段階問題）
  ...
  - グループCの内部は、5カテゴリのラウンドロビンで、未選択カテゴリを優先する
  - 30種類の新規テンプレート（比例・対応する量・反比例・対応する量・縮尺・実際の長さ・
    縮尺・地図上の長さ・縮尺を求める、各6種類）
  - トレーニングモードの5新規カテゴリにも対応（`proportion-corresponding-value`
    `inverse-proportion-corresponding-value` `scale-find-actual-length` `scale-find-map-length`
    `scale-find-scale`。比例・比例定数、反比例・一定の積という独立カテゴリは追加していない）
...
- 小学6年生3学期の内容（**第12段階で比例・反比例・縮尺の「対応する量を求める」5カテゴリのみ
...
- 比・拡大図・縮図（**第11段階で「比を使った数量」「比例配分」の2カテゴリ、
  第12段階で縮尺（実際の長さ⇔地図上の長さ⇔縮尺そのもの）の3カテゴリを追加**。
...
- 比・比の値・比例配分・比例・反比例・縮尺（**第11段階で比例配分・比を使った数量の2カテゴリ、
  第12段階で比例・反比例・縮尺の「対応する量を求める」5カテゴリを追加**。比を簡単にする問題・
...
- [ ] 6-2モードを選ぶと、分数の速さ・分数の道のり・分数の時間・分数割合・比べる量・分数割合・割合・
      分数割合・もとにする量・比を使った数量・比例配分・（復習として）4年生1〜3学期・5年生1〜3学期・
      6年生1学期の内容が出題される
...
- [ ] 10問（デフォルト出題数）を通しで見たとき、新内容（6年2学期の新出内容）と復習内容
      （4年生1〜3学期・5年生1〜3学期・6年生1学期の内容）が同数になっている
      （`?debug=true` のデバッグパネルで確認できる）
- [ ] 新内容の8カテゴリに極端な偏りがない（レベル5の新内容5問・レベルMAXの新内容6問が、
      毎回同じカテゴリの組み合わせにならない）
- [ ] 復習内容が4年生1〜3学期・5年生1〜3学期・6年生1学期から偏りなく出題される
- [ ] 同じカテゴリの問題（復習内容全体も1つのまとまりとして）が3問以上連続しない
...
- [ ] 6-3モードを選ぶと、比例・対応する量・反比例・対応する量・縮尺・実際の長さ・縮尺・地図上の長さ・
      縮尺を求める・（復習として）5年生1〜3学期・6年生1〜2学期の内容が出題される。**小学4年生の内容は
      出題されない**
...
- [ ] 縮尺・地図上の長さの答えが正しい（例: 縮尺1：50,000、実際の長さ2km→`2km=200000cm`、
      `200000÷50000=4cm`）
- [ ] 縮尺を求めるの答えが `1：n` の形式（`{type:"scale", numerator:1, denominator:n}`）で
      正しく表示される（例: 地図上4cm、実際1km→`1km=100000cm`、`100000÷4=25000`→`1：25,000`）
...
- [ ] グループCの新内容5カテゴリに極端な偏りがない（レベルMAXの新内容4問で、未選択カテゴリが
      優先して選ばれ、5カテゴリのうち4カテゴリが重複なく選ばれる）
...
- [ ] `tools/question-validator.html` で `gradeTerm` フィルターを `6-3` にすると、30件のテンプレートが
      すべてOK（NGが無い）と表示される
- [ ] `tools/question-validator.html` の関係表を持つテンプレートの例に、関係表が正しく表示される
- [ ] トレーニングモードの「がくねん・がっき」に「小学6年生・3学期」が表示され、5カテゴリ
      （比例・対応する量・反比例・対応する量・縮尺・実際の長さ・縮尺・地図上の長さ・縮尺を求める）を
...
- [ ] `generatorType` が `"findActualLengthFromScale"`／`"findMapLengthFromScale"`／`"findScale"`
      のいずれかになっている（3カテゴリとも `generateScaleLengthValues()` を共有する）
...
- [ ] 正解ルートは必ず1つで、2段階（長さの単位変換のステップを含む）になっている。単位変換の
      ...
      （m）が使われている
- [ ] 「縮尺を求める」の最終ステップには `resultType:"scaleDenominator"` を指定している
      （数値ではなく縮尺の値オブジェクトとして表示するため）
